...
*   `"extensible": true`：意味着这个对象是**开放的**。LLM可以用 `_.assign` 添加新键，或用 `_.remove` 删除键。这同时会导致该层下的所有子元素的required属性都被设置为 `false`，除非你在子元素中另行指定。
*   `"required": true`  (默认)：表示这个对象是**必需的**。不允许被 `_.remove` 删除。
*   `"required": false` ：表示这个对象是**可选的**。如果 LLM 认为这个对象不再需要，可以用 `_.remove` 删除它。
...
```json
{
  "$meta": { "extensible": true }, // 现在顶层结构是开放的，允许添加新角色
  "甲": {
    "$meta": { "required": true }, // 要注意的是，如果父节点被设置了extensible，那么这一层的子元素就允许增删，如果你需要规定这个元素不可删除，就必须特别设置required为true
    "状态A": [0, "描述..."],
    "状态B": [0, "描述..."]
  },
  "乙": {
    // 此处没有特别规定required为true，所以乙可以被删除
    "状态A": [0, "描述..."],
    "状态B": [0, "描述..."]
  }
}
...
如果你需要允许在根节点添加新角色，但如果你的角色变量中包含可变结构，那你需要提示LLM在添加新角色时，在对应的可变结构中设置 `"$meta": { "extensible: true }` 或者 `"$__META_EXTENSIBLE__$"`。
