...
`兼容性.额外模型解析非阻塞` 关闭时，使用原有的阶段过滤：在 `WORLDINFO_ENTRIES_LOADED` 中按调用时的
`is_during_extra_analysis`
完成全部筛选。不登记世界书请求、不向描述附加标记、不插入识别探针，也不依赖
`WORLDINFO_SCAN_DONE`。因此被排除的条目不会参与预算竞争或递归激活。

开启非阻塞后，才使用下述请求标记及两阶段扫描机制，区分重叠的主回复和额外解析。模式在每次条目加载时选择；已经插入探针的扫描仍按原快照完成，后续切换选项不会改变该扫描的请求归属。

两个世界书事件监听器在 MVU 实例初始化时注册，直到实例卸载才移除，不再随聊天级模块销毁和重建。回调仍受
`should_enable` 控制，避免非首选或已禁用实例参与过滤。

## 调用与识别

每次 `invokeExtraModel` 都分配或沿用自己的 `generation_id`。在非阻塞模式、MVU 已启用且
`is_during_extra_analysis` 为真时，登记在途请求及过滤配置快照，在 `finally`
中移除。成功、失败和取消都经过相同清理路径。

并发批次结束时，先取消批次信号，再按编号停止助手和 Pi 中已启动的生成，并等待各调用完成清理。世界书读取等待也响应取消；底层读取稍后返回时不会重新登记或启动生成。Pi 尝试在世界书读取前登记，过滤和模型调用共用同一份请求设置快照。
...
临时探针书名使用生僻字前缀 `龘靐齉`
加 5 位随机串，降低与用户世界书名称碰撞的概率。随机部分由 4 字节无符号随机数编码，每个字承载 7
bit；字母表为 CJK 扩展 A 的 128 种生僻字（U+3400–U+347F），不含 ASCII 字母、数字或连字符。
...
再作清理兜底。支持普通字符串和多模态消息的文本块，并移除原本仅含标记的空消息。最后一个登记调用结束后卸载标记清理监听器；实例级世界书监听器继续保留。

## 与提前过滤的差异

非阻塞模式实现请求归属隔离，但不承诺与阻塞模式的扫描前过滤完全等价：
...
单元测试覆盖阻塞模式的加载期过滤、无标记与无探针、实例级监听生命周期，以及非阻塞模式的配置不可变性、并发策略快照、普通请求隔离、未激活条目的整本支持判断、多轮清理、开关行为和消息标记清理。
...
```sh
npm test -- --runInBand tests/worldinfo_request_filter.test.ts tests/worldinfo_lifecycle.test.ts
