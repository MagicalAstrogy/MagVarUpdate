...
启用 MVU 和提示词模板插件后，可在发送给 AI 的世界书条目中用以下内容替换原有 JSON 变量宏：

```ejs
<%- mvuYaml() %>
```

输出某个子路径（路径相对于 `stat_data`，不需要加 `stat_data.` 前缀）：

```ejs
<%- mvuYaml('世界.时间') %>
```

同一函数也挂在酒馆父窗口的 `Mvu` 对象上，可写为 `<%- Mvu.mvuYaml('世界.时间') %>`，或显式使用 `<%- window.Mvu.mvuYaml() %>`。

默认每次调用都读取当前聊天最近一个含 MVU 数据的楼层及其当前 swipe，跳过没有变量的用户消息。若要使用模板的 `getvar` 缓存（例如同轮模板中刚修改过变量），可显式传入变量表：

```ejs
<%- mvuYaml('', getvar('stat_data')) %>
<%- mvuYaml('世界.时间', getvar('stat_data')) %>
```

空路径输出整个变量表；缺失或为 `null` 的子路径输出空字符串，`0` 和 `false` 正常保留。没有变量表时输出 `{}`。使用运行时的 `YAML.stringify` 序列化并去除首尾空白，不修改原始变量，也不改变原有 JSON 宏和变量更新格式。

请使用 `<%- ... %>`，避免 HTML 转义破坏 YAML；不启用提示词模板插件时，EJS 不会被展开。世界书中只保留所需的一份变量输出，避免 JSON 和 YAML 重复发送。
