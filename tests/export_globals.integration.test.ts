...
describe('export_globals integration test - Variable Update with display_data and delta_data', () => {
    ...
    test('should update stat_data, display_data and delta_data correctly for complex game state using Mvu.parseMessage', async () => {
        ...
        const initialState: MvuData = {
            initialized_lorebooks: ["工具书_ver1.1", "-YukiSecret"],
            stat_data: {
                剩余时间: 10,
                慕心: {
                    好感度: 0,
...
_.set('悠纪.当前所想[0]', "", "……狐狸？嗯…她的用词很精准。"); //回应慕心与远野亲昵举动后的思绪更新
        ...
        expect(updatedState.stat_data.慕心.当前所想[0]).toBe("狐狸真的很适合他呢，既聪明又带点狡猾…也只有我能画得出来");
        ...
        expect(updatedState.display_data.慕心.当前所想[0]).toBe("->狐狸真的很适合他呢，既聪明又带点狡猾…也只有我能画得出来 (基于她对远野的动作反馈)");
        expect(updatedState.display_data.悠纪.当前所想[0]).toBe("->……狐狸？嗯…她的用词很精准。 (回应慕心与远野亲昵举动后的思绪更新)");
        ...
        expect(updatedState.delta_data.慕心?.当前所想?.[0]).toBe("->狐狸真的很适合他呢，既聪明又带点狡猾…也只有我能画得出来 (基于她对远野的动作反馈)");
        expect(updatedState.delta_data.悠纪?.当前所想?.[0]).toBe("->……狐狸？嗯…她的用词很精准。 (回应慕心与远野亲昵举动后的思绪更新)");
    ...
    test('should use Mvu.setMvuVariable to update variables with display_data and delta_data', async () => {
        ...
        const result1 = await mvu.setMvuVariable(testData, 'player.health', 80, { reason: '受到伤害' });
        const result2 = await mvu.setMvuVariable(testData, 'player.mana', 75, { reason: '恢复法力' });
        const result3 = await mvu.setMvuVariable(testData, 'player.level', 6, { reason: '升级' });

        // 清理内部数据
        delete testData.stat_data.$internal;
    ...
    test('should use Mvu.getMvuVariable to get values from different data categories', () => {
        ...
        // 从 display_data 获取值
        const healthDisplay = mvu.getMvuVariable(testData, 'player.health', { category: 'display' });
        expect(healthDisplay).toBe("100->80 (受到伤害)");

        const positionDisplay = mvu.getMvuVariable(testData, 'player.position', { category: 'display' });
        ...
        const withDefault = mvu.getMvuVariable(testData, 'player.nonexistent', { default_value: 'default' });
...
`;
    ...
    test('Mvu.setMvuVariable with is_recursive should trigger eventEmit', async () => {
        ...
        // 测试 ValueWithDescription 类型的 is_recursive
        testData.stat_data.player.level = [10, "当前等级"];
        testData.display_data.player.level = [10, "当前等级"];
...
`;
        ...
        const healthDisplay = mvu.getMvuVariable(afterBatch, 'game.player.health', { category: 'display' });
