...
type MvuData = any;
type ExtractSetCommandTestOptions = { mvuZod?: boolean };

// 命令别名定义
const ASSIGN_ALIASES = ['assign', 'insert'] as const;
const REMOVE_ALIASES = ['remove', 'unset', 'delete'] as const;

export function registerExtractSetCommandTests({
    mvuZod = false,
    ...
    describe('extractCommands', () => {
        ...
        describe('嵌套括号处理', () => {
            ...
            test('处理嵌套数组和对象', () => {
                ...
                expect(parseCommandValue(cmd.args[1])).toEqual({
...
*/
    ...
    describe('高等数学与高级运算测试', () => {
        ...
        describe('插入元素后使用不同别名删除', () => {
            ...
            nativeTest.each(REMOVE_ALIASES)(
            ...
            nativeTest.each(allCombinations)(
                '使用 %s 插入后用 %s 删除（综合测试）',
                async (assignCommand, removeCommand) => {
                 ...
                 _.${assignCommand}('player.skills', 'block');//学会格挡`,
                    ...
                    expect(variables.stat_data.player.buffs).toEqual([
                        'strength',
                        'speed',
                        'defense',
                    ]);
            ...
            nativeTest('混合使用所有别名进行复杂操作', async () => {
                const variables = {
                    initialized_lorebooks: {},
                    stat_data: {
                        game: {
