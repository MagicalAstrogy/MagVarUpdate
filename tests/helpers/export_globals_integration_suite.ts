...
type MvuData = any;
type ExportGlobalsTestOptions = { mvuZod?: boolean };
...
export function registerExportGlobalsIntegrationTests({
    mvuZod = false,
}: ExportGlobalsTestOptions = {}): void {
    ...
    describe('export_globals integration test - Variable Update with display_data and delta_data', () => {
        ...
        nativeTest(
            'should update stat_data, display_data and delta_data correctly for complex game state using Mvu.parseMessage',
            async () => {
                // 初始状态
                const initialState = {
...
_.set('悠纪.当前所想[0]', "", "……狐狸？嗯…她的用词很精准。"); //回应慕心与远野亲昵举动后的思绪更新
                ...
                expect(updatedState.display_data.场景人物).toBe(
                    '未知->慕心&悠纪 (当前在场角色更新)'
                );
        ...
        nativeTest(
            'should emit COMMAND_PARSED with parsed command payload during parseMessage',
            async () => {
                const initialState = {
                    initialized_lorebooks: {},
                    stat_data: {
        ...
        test('should use Mvu.setMvuVariable to update variables with display_data and delta_data', async () => {
            ...
            const result3 = await mvu.setMvuVariable(testData, 'player.level', 6, {
                reason: '升级',
            });
        ...
        test('should use Mvu.getMvuVariable to get values from different data categories', () => {
            ...
            // 从 delta_data 获取值
            const healthDelta = mvu.getMvuVariable(testData, 'player.health', {
                category: 'delta',
            });
            expect(healthDelta).toBe('100->80 (受到伤害)');

            // 不存在的路径返回 undefined 或默认值
            const nonExistent = mvu.getMvuVariable(testData, 'player.position', {
                category: 'delta',
            });
...
`;
                ...
                expect(updatedState.display_data.player.position[0]).toBe(
                    '城镇->森林 (移动到森林)'
                );
        ...
        nativeTest('Mvu.parseMessage should handle number type conversions correctly', async () => {
            const testData = {
                initialized_lorebooks: {},
                stat_data: {
                    stats: {
...
// No actual updates
        ...
        nativeTest('Complex integration test with all Mvu methods', async () => {
            // 1. 初始化数据
            const initialState = {
                initialized_lorebooks: {},
                stat_data: {
...
`;
            ...
            await mvu.setMvuVariable(afterBatch, 'game.player.health[0]', 75, {
                reason: '战斗受伤',
            });
