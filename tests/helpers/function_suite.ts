...
export function registerFunctionTests({ mvuZod = false }: FunctionTestOptions = {}): void {
    ...
    describe('parseParameters', () => {
        ...
        describe('边界情况', () => {
            ...
            test('处理混合数据类型', () => {
                const result = parseParameters(
                    '"path", 123, true, null, [1, 2, 3], {key: "value"}'
                );
                expect(result).toEqual([
                    '"path"',
                    '123',
    ...
    describe('updateVariables', () => {
        ...
        nativeTest('应该更新变量并保留原始变量结构', async () => {
    ...
    describe('invokeVariableTest', () => {
        ...
        test('expect not updated', async () => {
            ...
            await handleVariablesInCallback(
                '这是一个没有更新的文本。明天见是最好的预言。',
                inputData
            );
            //现在会始终产生了。
    ...
    describe('strictSet feature', () => {
        nativeTest('strictSet=false 应该处理 ValueWithDescription 类型（默认行为）', async () => {
            const variables: MvuData = {
                stat_data: {
                    health: [100, '生命值'],
                    mana: [50, '魔力值'],
        ...
        nativeTest('strictSet=true 应该允许替换整个数组', async () => {
            const variables: MvuData = {
                stat_data: {
                    items: [10, '物品数量'],
                },
        ...
        nativeTest('strictSet=false 保持数组描述不变', async () => {
            const variables: MvuData = {
                stat_data: {
